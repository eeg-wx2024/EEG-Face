...
class KNNClassifier(nn.Module):
    ...
    def forward(self, x, topk=5):
        ...
        if topk == 1:
            ...
            return torch.from_numpy(pred).float().to(x.device).unsqueeze(1)
        else:
            # 返回Top-k预测的概率和索引
            distances, indices = self.model.kneighbors(x_np, n_neighbors=topk)
            # 获取最近邻的标签
            topk_preds = self.model._y[indices]  # shape: (n_samples, topk)
            return torch.from_numpy(topk_preds).float().to(x.device)
...
class OldNet(nn.Module):
    ...
    def forward(self, x, topk=1):
        return self.net(x, topk=topk) if hasattr(self.net, 'forward') and 'topk' in self.net.forward.__code__.co_varnames else self.net(x)
