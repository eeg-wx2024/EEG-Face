...
def map_labels_to_binary(labels):
    # 定义映射规则
    positive_labels = {1, 2, 6, 7, 9, 10, 12, 13, 17, 21, 22, 24, 26, 27, 33, 35, 37, 38, 39}
    binary_labels = torch.zeros_like(labels)
    for i, label in enumerate(labels):
        if label.item() in positive_labels:
            binary_labels[i] = 1
...
def main():
    ...
    # 将标签转换为二分类
    all_labels = map_labels_to_binary(all_labels)
    ...
    for fold, (train_idx, val_idx) in enumerate(k_fold.split(all_eeg_data)):
        ...
        # 模型
        model = select_model("nice").to(device)
        ...
        for epoch in range(args.epochs):
            ...
            for step, (x, y) in enumerate(train_loader):
                total_sample += x.shape[0]  # 累加当前批次的样本数
                x = x.to(device)
                y = y.to(device)
                optimizer.zero_grad()
                y_pred = model(x)
                loss = F.binary_cross_entropy_with_logits(y_pred.squeeze(), y.float())
                loss.backward()
                optimizer.step()
            ...
            for step, (x, y) in enumerate(val_loader):
                ...
                y = y.to(device)
                y_pred = model(x)
                predicted = (y_pred.squeeze() > 0.5).long()  # 将概率转换为 0 或 1
                total_correct += (predicted == y).sum().item()
                total_sample += y.size(0)
            val_acc = total_correct / total_sample * 100

            recent_acc.append(val_acc)

            if val_acc > best_val_acc:
                best_val_acc = val_acc
                best_epoch = epoch

            if val_acc > best_top5_acc:
                best_top5_acc = val_acc
                best_epoch5 = epoch

            writer.add_scalar("fold {}/val/loss".format(fold), val_acc, epoch)
            writer.add_scalar("fold {}/val/acc".format(fold), val_acc, epoch)
            writer.add_scalar(
            ...
            if len(recent_acc) > 4:
                ...
                recent_mean = 0
            print(
                f"fold {fold}, epoch {epoch}, train_loss {train_loss:.4f}, val_loss {val_acc:.2f}, train_acc {train_acc:.2f}, val_acc {val_acc:.2f}, best_val_acc {best_val_acc:.2f}, best_top5_acc {best_top5_acc:.2f}, recent_mean {recent_mean:.2f} (epoch {best_epoch}/{epoch}, epoch5 {best_epoch5}/{epoch})"
            )
        ...
        with open(log_file, 'a+') as f:
            f.write(f"fold {fold}, epoch {epoch}, train_loss {train_loss:.4f}, val_loss {val_acc:.2f}, train_acc {train_acc:.2f}, val_acc {val_acc:.2f}, best_val_acc {best_val_acc:.2f}, best_top5_acc {best_top5_acc:.2f}, recent_mean {recent_mean:.2f} (epoch {best_epoch}/{epoch}, epoch5 {best_epoch5}/{epoch})\n")
    with open(log_file, 'a+') as f:
