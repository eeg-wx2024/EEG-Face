...
def getconditions(x):
    ...
    min_values = x.amin(dim=[2, 3])
    ranges = max_values - min_values
    condition = ranges < 100
    condition = condition.to(device).squeeze()
    return condition
...
def main():
    ...
    with open(log_file, 'a+') as f:
        f.write(f"Top-1 Accuracies (3 folds): {best_accs}\n")
        f.write(f"Top-5 Accuracies (3 folds): {best_accs5}\n")
        f.write(f"Mean Accuracies (3 folds): {final_accs}\n")
