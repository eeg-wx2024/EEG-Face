...
def main():
    ...
    with open(log_file, log_mode) as f:
        f.write(f"Starting training for n=2 to n=39 at {timestamp}\n")
    ...
    for n in range(2, 40):
        ...
        selected_labels = torch.randperm(len(unique_labels))[:n]
        selected_labels = unique_labels[selected_labels]

        # 过滤数据，仅保留选中的标签
        mask = torch.isin(all_labels, selected_labels)
        filtered_eeg_data = all_eeg_data[mask]
        filtered_labels = all_labels[mask]

        # 将数据移到 GPU
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        filtered_eeg_data = filtered_eeg_data.to(device)
        ...
        best_accs = []
        best_accs5 = []
        for fold, (train_idx, val_idx) in enumerate(k_fold.split(filtered_eeg_data)):
            train_data = filtered_eeg_data[train_idx]
            val_data = filtered_eeg_data[val_idx]
            train_labels = filtered_labels[train_idx]
            val_labels = filtered_labels[val_idx]

            val_paths = []
            sample_count = 0
            for idx in val_idx:
                sample_idx = idx
                for i, data_len in enumerate(file_data_lengths):
                    sample_count += data_len
                    if sample_idx < sample_count:
                        base_name = list(edf_files.keys())[i]
                        val_paths.append(edf_files[base_name]['edf'])
                        break
            ...
            for epoch in range(args.epochs):
                ...
                total_sample = 0
                save_dir = get_unique_folder(fold, epoch)
        ...
        with open(log_file, 'a+') as f:
            f.write(f"labels: {n}\nTop-1 Accuracies (5 folds): {best_accs}\nTop-5 Accuracies (5 folds): {best_accs5}\nMean Accuracies (5 folds): {final_accs}\nMean Accuracies (5 folds): {final_accs}\n\n")
